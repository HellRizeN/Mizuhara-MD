...
sendOrder(m.chat, teks, "5123658817728409", fs.readFileSync('./Chizuru Mizuhara/pic10.jpg'), `${watermark}`, `${BotName}`, "916909137213@s.whatsapp.net", "AR7zJt8MasFx2Uir/fdxhkhPGDbswfWrAr2gmoyqNZ/0Wg==", "99999999999999999999")
}
        ...
        if (tebakkata.hasOwnProperty(m.sender.split('@')[0]) && isCmd) {
          ...
          if (kuismath.hasOwnProperty(m.sender.split('@')[0]) && isCmd) {
            ...
            if (isMedia && m.msg.fileSha256 && (m.msg.fileSha256.toString('base64') in global.db.sticker)) {
              ...
              const textImg = (teks) => {
                Miku.sendMessage(m.chat, { text :teks, }, {quoted: m, thumbnail: fs.readFileSync('./Chizuru Mizuhara/pic4.jpg')}) 
                }
                ...
                const fgi = {
                  ...
                  message: { 
                                ...
                                'gifPlayback': 'true', 
                                'caption': `Scotch`,
                                'jpegThumbnail': fs.readFileSync('./Mizuhara/HellRizeN.mp4')
                                       }
                                      }
                                   } 
...
const ftroli = {
  ...
  const jumlahUser = pendaftar.length
    ...
    const menulist = `
    Konichiwa ${pushname} dear 👋. I am ${global.BotName}, a bot developed by: Scotch to take your WhatsApp usage into next level.
...
//Some special chat replies
 ...
 let smallinput = budy.toLowerCase()
    ...
    if (smallinput=='bot') {
      reply (`Hello *${pushname}*, I am *${BotName}*, a WhatsApp bot made by *Scotch* and currently being hosted by *${OwnerName}*.  type  *${prefix}help* to get my full command list.`);
    }
...
switch(command) {
    ...
    teks = `*${global.BotName} : Ooop Sorry Error 404 Bye Bye...*`
    let buttons = [
    ...
    body: " ", 
    thumbnail: fs.readFileSync("Chizuru Mizuhara/pic2.jpg"),
    mediaType:1,
...
case 'repo': case 'botrepo':
    ...
    reply(`*My Source Code :* *Ooop Error 404 Bye Bye....* ❤️`)
    break
...
const getCase = (cases) => {
return "case"+`'${cases}'`+fs.readFileSync("MizuCore.js").toString().split('case \''+cases+'\'')[1].split("break")[0]+"break"
}
replay(`${getCase(q)}`)
break
...
await Miku.sendMessage(m.chat, { delete: key })
 ...
 let teks = ` 「  Mizuhara's group user list  」\n\nTotal ${anu.length} users are using bot in Groups.`
 for (let i of anu) {
...
let mentioned = participants.map(v => v.jid)
 ...
 case 'resetlinkgc':
    ...
    case 'group': case 'grup': {
     ...
     sendOrder(m.chat, teks, "667140254502463", fs.readFileSync('./Chizuru Mizuhara/pic7.jpg'), `${global.packname}`, `${global.BotName}`, "916909137213@s.whatsapp.net", "AR6NCY8euY5cbS8Ybg5Ca55R8HFSuLO3qZqrIYCT7hQp0g==", "99999999999999999999")
     } else if (sizny > 20) {
     await Miku.groupAcceptInvite(vcc).then(async(res) => replay(jsonformat(res))).catch(_ => _)
     replay("Joined !")
     } else {
...
case 'music': case 'play': case 'song': case 'ytplay': {
 ...
 caption: `「  _Mizuhara Youtube Player 2.0_  」
...
*Url :* ${anu.url}`,
 ...
 Miku.sendMessage(from, {video:{url:ytmp4play.videoUrl}, mimetype:"video/mp4", caption:anu.title+' By *Mizuhara MD*',}, {quoted:m})
 }
 break
 ...
 let buttonMessage = {
 image: { url: anu.thumbnail },
 caption: `「  _Mizuhara Youtube Downloader 2.0_  」
...
*Url :* ${anu.url}`,
 ...
 await Miku.sendMessage(from, {document: fs.readFileSync(ytmp3play2.path),fileName:'Mizuhara_YTmp3_Downloader.mp3',mimetype: 'audio/mpeg',}, {quoted:m})
 }
 break
 ...
 const YT=require('./lib/ytdlcore')
    ...
    if (vidduration > 1800) return reply('Cant send videos longer than *30 min*')
 Miku.sendMessage(from, {video:{url:ytmp4play2.videoUrl}, mimetype:"video/mp4", caption:'Downloaded by *Mizuhara MD*',}, {quoted:m})
 }
 break
...
case 'swm': case 'take': case 'stickerwm': case 'steal':{
    if (isBan) return reply(mess.banned)
    if (isBanChat) return reply(mess.bangc)
if (!args.join(" ")) return reply(`Use command: -steal Mizuhara|By : Scotch`)
const swn = args.join(" ")
...
Mizu.sendMessage(from, { text: `*${command}*\n\nName : ${q}\nAnswer : *${teng}%*` }, { quoted: m })
					break
case 'beautifulcheck':
    if (isBan) return reply(mess.banned)
    if (isBanChat) return reply(mess.bangc)
...
case 'charactercheck':
    ...
    if (isBanChat) return reply(mess.bangc)
					...
					Mizu.sendMessage(from, { text: `Character Check : ${q}\nAnswer : *${taky}*` }, { quoted: m })
				     break
 ...
 case 'dare':
    ...
    if (isBanChat) return reply(mess.bangc)
         ...
         "say i love the bot creator Scotch through voice note",
         ...
         "breakup with your best friend for 5hrs without telling him/her that its a dare",
          ...
          "send abusive words in any grup, excepting this grup, and send screenshot proof here"
                   ...
                   buffer = await getBuffer(`https://wallpapercave.com/uwp/uwp2679279.jpeg`)
...
case 'truth':
    ...
    if (isBanChat) return reply(mess.bangc)
                 ...
                 "do you play pubg, if you then send ur id number"
                           ...
                           buffer = await getBuffer(`https://wallpapercave.com/wp/wp6446709.jpg`)
                           Mizu.sendMessage(from, { image: buffer, caption: '*You have chosen Truth*\n'+ mikutruthww }, {quoted:m})
                           break
...
Mizu.sendMessage(from, {image:{url:nye}, caption:"Master..."}, {quoted:m})
break
...
*Link* : ${baby1[0].link}`
reply(`${result4}`)
Mizu.sendMessage(m.chat, { document : { url : baby1[0].link}, fileName : baby1[0].nama, mimetype: baby1[0].mime }, { quoted : m }).catch ((err) => reply(mess.error))
}
break
...
Mizu.sendMessage(from, {image:YesHorny},{quoted:m})
} catch (e) {error("Error")}	
break
...
let spgif = await GIFBufferToVideoBuffer(spbuff)   
        await Mizu.sendMessage(m.chat,{video: spgif, gifPlayback:true},{ quoted:m }).catch(err => {
                    return reply('Error!')
                                    })
break
...
let bjif = await GIFBufferToVideoBuffer(bjf)   
        await Mizu.sendMessage(m.chat,{video: bjif, gifPlayback:true},{ quoted:m }).catch(err => {
                    return reply('error..')
                                    })
break
...
Mizu.sendMessage(m.chat, { video: { url: result912.video_1 }, caption: `Title : ${result912.title}\nCategory : ${result912.category}\n$Mimetype : ${result912.type}\nViews : ${result912.views_count}\nShares : ${result912.share_count}\nSource : ${result912.link}\nMedia Url : ${result912.video_1}` }, { quoted: m })
}
break
...
reply(mess.waiting)
 ...
 let trapbot = [
  ...
  headerType: 1
            ...
            await Mizu.sendMessage(m.chat, button2Messages, { quoted:m }).catch(err => {
                    return('Error!')
                })
break
...
reply(mess.waiting)
 ...
 let hnekobot = [
  ...
  headerType: 1
            ...
            await Mizu.sendMessage(m.chat, button3Messages, { quoted:m }).catch(err => {
                    return('Error!')
                })
break
...
reply(mess.waiting)
 ...
 let nwaifubot = [
  ...
  headerType: 1
            ...
            await Mizu.sendMessage(m.chat, button4Messages, { quoted:m }).catch(err => {
                    return('Error!')
                })
break
...
reply(mess.waiting)						
 waifudd = await axios.get(`https://nekos.life/api/v2/img/${command}`)
      ...
      headerType: 4
            ...
            await Mizu.sendMessage(m.chat, buttonsssMessages,{ quoted:m }).catch(err => {
                    return('Error!')
                })
break  
...
reply(mess.waiting)						
 waifudd = await axios.get(`https://nekos.life/api/v2/img/smug`)
      ...
      headerType: 4
            ...
            await Mizu.sendMessage(m.chat, button1ssMessages,{ quoted:m }).catch(err => {
                    return('Error!')
                })
break
...
reply(mess.waiting)							
 waifudd = await axios.get(`https://nekos.life/api/v2/img/fox_girl`)
      ...
      headerType: 4
            ...
            await Mizu.sendMessage(m.chat, button12ssMessages,{ quoted:m }).catch(err => {
                    return('Error!')
                })
break   
...
reply(mess.waiting)
 ...
 let xxhnekobot = [
  ...
  headerType: 1
            ...
            await Mizu.sendMessage(m.chat, xx1button3Messages, { quoted:m }).catch(err => {
                    return('Error!')
                })
break
...
reply(mess.waiting)						
 waifudd = await axios.get(`https://nekos.life/api/v2/img/waifu`)
      ...
      headerType: 4
            ...
            await Mizu.sendMessage(m.chat, button112ssMessages,{ quoted:m }).catch(err => {
                    return('Error!')
                })
break
...
case 'crossplay': case 'crosplay': case 'cosplay':
    ...
    if (!m.isGroup) return replay(mess.grouponly)
        ...
        await Mizu.sendMessage(m.chat,cosplybutton, { quoted:m }).catch(err => {
            return('Error!')
        })  
...
reply(mess.waiting)							
   waifud = await axios.get('https://waifu.pics/api/sfw/neko')
      ...
      headerType: 4
            ...
            await Mizu.sendMessage(m.chat,buttonssMessage, { quoted:m }).catch(err => {
                    return('Error!')
                })               
                break
...
reply(mess.waiting)							
 waifudd = await axios.get(`https://nekos.life/api/v2/img/${command}`)
      ...
      headerType: 4
            ...
            await Mizu.sendMessage(m.chat, buttonssMessages,{ quoted:m }).catch(err => {
                    return('Error!')
                })
break
...
console.log(musers)
		...
		Mizu.sendMessage(m.chat,{video: fetchedgif, gifPlayback:true,mentions:ment,caption:musers},{quoted:m})
    } catch (error) {
        console.log(error);
    }
}
...
let resmain = await GIFBufferToVideoBuffer(resffj)   
      await Mizu.sendMessage(m.chat,{video: resmain, gifPlayback:true},{ quoted:m }).catch(err => {
                  return reply('error..')
                                  })
break
...
await Mizu.sendMessage(m.chat,buttonzMessage, { quoted:m }).catch(err => {
     return('Error!')
    })               
break     
...
case 'awoo':
 ...
 var wbuttsss = [
  ...
  headerType: 2
            ...
            await Mizu.sendMessage(m.chat, button1Messages, { quoted:m }).catch(err => {
                    return('Error!')
                })
break
...
var walb = [
      ...
      headerType: 4
            ...
            await Mizu.sendMessage(m.chat, wal,{ quoted:m }).catch(err => {
                    return('Error!')
                })          
break
...
Mizu.sendMessage(m.chat,{image:{url:result.images.jpg.large_image_url},caption:details},{quoted:m})   
break
...
Mizu.sendMessage(m.chat,{image:{url:srh.data[0].images.jpg.large_image_url},caption:mang},{quoted:m})   
break
...
reply(mess.waiting)	
 ...
 var wbuttsssr = [
    ...
    await Mizu.sendMessage(m.chat, button4Messagess, { quoted:m }).catch(err => {
        return('error..')
        })
break
...
reply(mess.waiting)	
 ...
 var wbuttsssr = [
    ...
    await Mizu.sendMessage(m.chat, buttonMessagessf, { quoted:m }).catch(err => {
        return('error..')
        })
break
...
reply(mess.waiting)	
 ...
 var wbuttsssr = [
    ...
    await Mizu.sendMessage(m.chat, buttonMessagessfgr, { quoted:m }).catch(err => {
        return('error..')
        })
break
...
teks = "\nDarkjokes"
Mizu.sendMessage(m.chat, { image : { url : res }, caption: teks }, { quoted : m })
break
...
case 'leavegc': case 'leavegroup': case 'bye': {
    ...
    if (!m.isGroup) return replay(mess.grouponly)
        reply(mess.waiting)
                    if (!isCreator) return replay(`${mess.botowner}`)
                    await Mizu.groupLeave(m.chat).then((res) => reply(jsonformat(res))).catch((err) => reply(jsonformat(err)))
                }
                break
...
Mizu.send5ButImg(yoi, txt, `${global.BotName}`, BotLogo, btn, Thumb)
}
...
case '':
    ...
    if (isBanChat) return reply(mess.bangc)
      ...
      mizupic ='https://c.tenor.com/cHyx0ESAuIUAAAAC/mami-chizuru-oppai.gif'
    
        
 const needhelpmenu = `Hello... ${pushname} Senpai? Have a great day 🎉.`
     
         let butRun = [
                {buttonId: `${prefix}help`, buttonText: {displayText: 'Menu'}, type: 1}
                ]
                let buttonMessage = {
                    video:fs.readFileSync('./Mizuhara/Horny.mp4'),gifPlayback:true,
                    caption: needhelpmenu,
                    footer: `${global.BotName}`,
                    buttons: butRun,
                    headerType: 4
                }
            Mizu.sendMessage(m.chat,buttonMessage,{quoted:m})
                }
break
...
const txt = `Do you love Chizuru? Then we are the same.`
const Mizuhara= [
            ...
            const Mizuharaselection = Mizuhara[Math.floor(Math.random()*Mizuhara)]
        
            Mizu.sendMessage(from,{video:{url:Mizuharaselection},gifPlayback:true,caption:txt},{quoted:m})
